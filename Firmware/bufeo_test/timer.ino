...
void task_timer(void *arg)
{
    ...
    while (1)
    {
        ...
        if (insideMenuFlag)
        {
            ...
            Period = 60000 / ((int)currentFrecRespiratoria);
            activePeriod = (float)(60000.0/ (float)currentFrecRespiratoria)/(1+(float)((float)currentRelacionIE / 10.0));

            contPeriod++;
            if (contPeriod >= Period)
            {
                contPeriod = 0;

                Serial.print(Period);
                Serial.print(", ");
                Serial.println(activePeriod);
            }

            if (contPeriod < activePeriod)
            {
                flagActive = true;
            }
            else
